...
class ReferencesController < ApplicationController
  ...
  def index
    # index is now all the references for a single topic (not all the references)
    # the URL will contain the id of the current topic (:topic_id)
    # use the :topic_id parameter to find all the references
    @topic = Topic.find params[:topic_id]
    # the has_many function called in app/models/topic.rb created the 
    # function "references" which will return an array of all the references 
    # with the topic foreign key matching @topic
  ...
  def destroy
    @reference.destroy
    redirect_to topic_references_url(@reference.topic) , notice: 'Reference was successfully destroyed.'
  end
